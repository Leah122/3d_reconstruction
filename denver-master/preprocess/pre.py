...
def filter_extract(dir_name="CVAI-2828RAO2_CRA32", base_path="datasets"):
    ...
    for i, filename in tqdm(enumerate(filter_images), total=len(filter_images), desc="Binarize"):
        if filename.endswith(".png") or filename.endswith(".jpg"):
            ...
            # loop over each pixel, for each bright pixel do a region growing algorithm.
            ...
            #  keep the most bright region
            max_intensity_index = np.argmax(intensities)
            for x, y in connected_regions[max_intensity_index]:
                image_processed[x, y] = 255
            
            # loop over all regions and keep the ones that are have a sum of intensities higher than 1000

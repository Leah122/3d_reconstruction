...
def main(args):
    ...
    # run raft
    cmd = f"cd scripts && python3 dataset_raft.py  --root ../custom_videos/ --dtype custom --seqs {data_name}"
    print(cmd)
    subprocess.call(cmd, shell=True)

    # stage 1
    cmd = f"python3 nir/booststrap.py --data {data_name}"
    print(cmd)
    subprocess.call(cmd, shell=True)
    # stage 2
    cmd = f"python3 run_opt.py data=custom data.seq={data_name}"
    print(cmd)
    subprocess.call(cmd, shell=True)
